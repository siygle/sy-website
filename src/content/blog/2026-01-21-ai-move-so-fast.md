...
裡面除了單純用 Astro 改寫之外，裡面有一個比較特別的需求，就是「針對 X、bluesky、YouTube 的 markdown link -> embedded 的轉換」這件事，看起來就是支援能嵌入社交平台的文章，但我因為把寫作的流程改到 Obsidian 上面，為了介面友善的目的就選用搭配下列這個 plugin，它的功能是可以讓貼上的 sns 連結展開成 embedded post 的形式。
...
但對 markdown 而言它實際上只是利用插入圖片的格式，所以用[這篇推文](https://x.com/jack/status/20)來舉例就會是下面這種格式：
```
![](https://x.com/jack/status/20)
```

但這個方式當你佈署上線之後，它是不會轉譯成你預覽看到的 embedded post 的格式，因為對 markdown 來說它其實是**顯示圖片**的語法，正確的嵌入推文應該是：
```
...
所以你會需要一個在 static website generator 中用來處理轉換的行為來能正常顯示這些 sns 文章。如果是前端苦手大概就是放棄，或是找其他現成的工具，但因為有 AI 我就試著把需求描述給它，它分析了之後就用 [remark plugin 的方式來實作](https://github.com/siygle/sy-website/blob/master/src/lib/remark-social-embed.ts)就實作出我希望達到的效果了。原本可能會是「找第三方」->「測試」這個流程的反覆幾次，現在一個 AI 來回就完成了。

的確，它可能不會是一個多少人會用的模組，但這種極小的需求，正是上述 Barefoot Developet 所描述的場景。之前有人形容未來的 AI 算力就會變成像現今的水、電一樣基本生存必要的元件，這樣看起來軟體人可能就是未來的水電師傅吧～😂
...
幾年前，甚至說以往，開發者對於各式各樣的開發工具經常趨之若鶩，這也是一塊許多人投入的市場，收費產品、開源專案等多不勝數。光看開發者熟知的 VSCode，圍繞在它身旁的就有多少 extensions，但最近 agent 興起之後，不少都轉移以 agent 為主、編輯器為輔的工作模式，當這個趨勢持續下去之後，原本的那些開發工具是不是會越來越不重要，反而是瀏覽、管理等工具會興起呢？
...
大家好像樂此不疲對於這類程式語言比較的問題，也常常帶出許多討論的流量。但最近一直在思考，如果未來 AI 真的接手開發者大部分手動輸入程式的工作，那爭論什麼語言還有很大的意義嗎？想想現在很多語言的區分，不外乎就是 syntax、效能、學習曲線等問題各有優劣，但對 AI 來說多數這類可能都不再是問題，有沒有可能之後 AI 都直接針對底層結構直接處理，我們只需要針對「輸出的結果」來驗證並調校，再也不需要程式語言轉譯這一層了？
...
想像目前 iOS、Android 之所以沒有太多競爭者，一個很大的原因是它們背後那龐大的生態圈，想要替代它並不是那麼簡單的一件事。但如果今天有人推出了一個「完全透過 AI 來開發應用程式」的平台會發生什麼事呢？今天應用程式之所以會存在，一來並不是所有人都有思考、規畫、設計的能力，再者就算有，也不是每個人都有辦法將想法實作出來。
...
以往要加入這些功能，要嘛期待官方自己開發、要嘛就是找有支援類似功能的第三方來使用，但假使之後每個使用者都可以直接描述自己的需求，系統就自動產生出一款專屬於你需求的軟體。如果真能如此，「軟體開發」這件事會不會又是以完全不一樣的形式存在呢？

---
***實在有太多太多的可能性，隨著 AI 那驚人進化的速度，可以無限展開。它以我們無法想像的速度在進化中，也逐漸改變我們熟知的一切。如今的我們，或許正處於一個最壞、也是最好的時刻，我們可以無力、沮喪、悲觀看待這一切，也可以試著理解並思考下一步該怎麼走。Never stop thinking***
